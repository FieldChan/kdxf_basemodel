...
model = lgb.LGBMClassifier(boosting_type='gbdt', num_leaves=48, max_depth=-1, learning_rate=0.05, n_estimators=2000,
                           ...
                           colsample_bytree=1, reg_alpha=3, reg_lambda=5, seed=1000, nthread=10, silent=True)
...
for i, (train_index, test_index) in enumerate(skf.split(X_loc_train, y_loc_train)):
    ...
    baseloss.append(lgb_model.best_score['valid']['binary_logloss'])
    loss += lgb_model.best_score['valid']['binary_logloss']
    test_pred= lgb_model.predict_proba(X_loc_test, num_iteration=lgb_model.best_iteration)[:, 1]
...
print('Process finished, time cost:',time_end-time_start,'s')